...
/**
 * Description: 将中缀表达式转化为后缀表达式，再通过栈来计算表达式的值
 *
 * @author lyx
...
public class Calculate {
   ...
   public Fraction outcome(String s) {
    ...
    //生成后缀表达式
    public Queue<String> toSuffixExpression(String s) {
        ...
        while (!queue.isEmpty() ) {
            ...
            if (isDigital(s.charAt(0))) {
             ...
             //   System.out.println("压入栈的f："+f.getNumerator()+"/"+f.getDenominator());
                ...
                * */
                if ( c == '-' ) {
                    f = f2.subtract(f1);//若结果为负，break；返回-1，用以标记表达式不合法
                    f.Appointment();//简单约分
